...
$sql = "SELECT w.wdr_id,m.mem_email,w.mem_id,w.wdr_accountname,w.wdr_bank,w.wdr_accountnumber,w.wdr_amount,w.wdr_status,w.wdr_created";
$sql.=" FROM withdrawal w INNER JOIN member m ON w.mem_id=m.mem_id";
$query=mysqli_query($conn, $sql) or die("listsaldo.php: 1get employees");
$totalData = mysqli_num_rows($query);
	...
	// preparing an array
		...
		if($row["wdr_status"]=="Pending"){
			$nestedData[] = 
			" <a href='?idw=".$row["wdr_id"]."' class='tombol' style='background-color: #d6d94f;'>PROSES</a>";
		}else{
			$nestedData[] = 
			"<a class='tombol'>".$row["wdr_status"]."</a>";
		}
