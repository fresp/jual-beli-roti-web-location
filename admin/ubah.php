...
<body style="background-color: #ECF0F5;">
	...
	<div id="panel-content">
		<?php
		if($_GET['jenis']=="member" && $_GET['email']){
			?>
			...
			$member = new member();
			$adminData = $member->ubah($username); 
			echo $adminData;
		}elseif($_GET['jenis']=="admin" && $_GET['username']){
			$username =$_GET['username'];
			require_once '../module/admin.php';
			$admin = new admin();
			$adminData = $admin->ubah($username); 
			echo $adminData;

		}else{
			$_SESSION['alert'] = "Notfound";
