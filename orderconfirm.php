...
<body style="background-color: #F5F5F5;">
      ...
      <div class="span12">
        <form id="customerform" method="post" class="form-customer">  
          <div class="row-fluid">
            ...
            <div >
              <input type="text" name="penerima" class="txtfield  input-xxlarge"  autocomplete="off" placeholder="Nama Penerima">
            </div>
            <label class="control-label" for="inputAddress">Alamat Lengkap</label>
            <div >
              <textarea type="text" name="alamat" class="txtfield  input-xxlarge"  autocomplete="off" placeholder="Jl.Pejaten Barat II
              Rt 011/08 No.23
              Patokan Rumah Freza Nugraha"></textarea>
            </div>
            <label class="control-label" for="inputAddress">Nomer Lain yang dapat dihubungi</label>
            <div >
              <input type="text" name="nomer" class="txtfield  input-xxlarge"  autocomplete="off" placeholder="+629999999999">
            </div>
          </div>
 ...
 <div style="margin-top: 50px;background: #fff;padding: 10px;border-bottom: 3px solid #eee;" class="container container-top">
  ...
  <div class="feed" id="feed">
    ...
    <span id="erroralamat" style="font-weight: bold; float: right;color: red" ></span>
  </div>
</div>
<div style="background: #fff;padding: 10px;" class="container container-top">
  ...
  <div class="feed" id="feed" style="text-align: center;margin-top: 7px;">  
   <a class="btn-pesan" href="#loginmodal" id="modaltrigger" style="float: none;"><i class="fa fa-home"></i>  Tambah Alamat Baru</a>
 </div>
...
<div style="background: #fff;padding: 10px;margin-top: 7px;" class="container container-top">
  ...
  <div id="frm-pesan" style="display: none;">
    <div class="search-wrapper" id="carii" style="margin-top: 0px;margin-left: 4px;">
      <textarea type="text" id="pesan" style="width: 100%" placeholder="isi Pesan Mu disini"></textarea>
      <script type="text/javascript">
...
<div style="background: #fff;padding: 10px;height: 70px;" class="container container-top">
  ...
  <div class="column-6" style="float: right;text-align:right;padding-right: 0px">
    ...
    <span class="hasilfree" style="font-weight: bold;display: none;"></span>
    <script type="text/javascript">
      var sum = 0.0;
      var total = 0.0;
  ...
  <script type="text/javascript">
    $("input[type='radio']").click(function(){
      ...
      if(a=="cod"){
        ...
        var hasil = sum;
        $(".tampilongkir").html(Number(hasil+(hasil*diskon)).toLocaleString('id'));
        $(".hasilongkir").html(hasil+(hasil*diskon));
        $("#alert").html('Biaya Tambah Ongkir 30%');
        var konfirm = 0.0;
        $('.konfirmasi').each(function()
        {
          konfirm += parseFloat($(this).text());
        });
        $(".hasilkonfirmasi").html(konfirm);
        $(".totalkonfirmasi").html(Number(konfirm).toLocaleString('id'));
        var r = 0.0;
        ...
        $("#errorpay").remove();
        $('#metode').css('border', '0px solid #df3434');
        ...
        $("#alert").html('Kode Unik Kamu :'+ randomNumber);
        var hasil = sum;
        $(".tampilongkir").html(Number(hasil).toLocaleString('id'));
        $(".hasilongkir").html(hasil);
        var konfirm = 0.0;
        $('.konfirmasi').each(function()
        {
          konfirm += parseFloat($(this).text());
        });
        $(".hasilkonfirmasi").html(konfirm+randomNumber);
        $(".totalkonfirmasi").html(Number(konfirm+randomNumber).toLocaleString('id'));
        var r = 0.0;
        $('.hasilkonfirmasi').each(function()
        {
          r += parseFloat($(this).text());
...
<div style="background: #fff;padding: 7px; margin-top: 7px;text-align: center;" class="container container-top">
  <form action="helper/payment.php?aksi=cartselected" id="myForm" method="POST">
    ...
    <input type="hidden"  id="isicust" name="isicust"/>
    ...
    $(function() {
      $("#myForm").submit(function() {
        $("#payment").each(function() {
          ...
          if (!cust) {
            $('<span>Lokasi Pengiriman Kosong</span>').appendTo('#erroralamat');
            $('#alamat').css('border', '2px solid #df3434');
